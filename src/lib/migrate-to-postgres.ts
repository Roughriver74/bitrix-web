...
import { Course, Lesson, Test, TestQuestion } from '@/types'
...
export async function migrateToPostgres() {
	try {
      ...
      `
		...
		const tests = db.prepare('SELECT * FROM tests').all() as Test[]
		for (const test of tests) {
			await sql`
        INSERT INTO tests (course_id, lesson_id, title, description, created_at)
        VALUES (${test.course_id}, ${test.lesson_id || null}, ${test.title}, ${
				test.description
			}, ${test.created_at})
        ON CONFLICT DO NOTHING
      `
		...
		const questions = db
			.prepare('SELECT * FROM test_questions')
			.all() as TestQuestion[]
		for (const question of questions) {
			await sql`
        INSERT INTO test_questions (test_id, question, options, correct_answer, order_index)
        VALUES (${question.test_id}, ${question.question}, ${JSON.stringify(
				question.options
			)}, ${question.correct_answer}, ${question.order_index})
        ON CONFLICT DO NOTHING
      `
		}
