...
export async function POST(request: NextRequest) {
	try {
		...
		const user = db
			...
			| { id: number; email: string; name: string; is_admin: boolean }
			| undefined
		if (!user || !user.is_admin) {
			return NextResponse.json({ error: 'Нет прав доступа' }, { status: 403 })
		}

		const { test_id, question, options, correct_answer, order_index } =
			await request.json()
    ...
    `
		...
		const questionData = db
			.prepare('SELECT * FROM test_questions WHERE id = ?')
